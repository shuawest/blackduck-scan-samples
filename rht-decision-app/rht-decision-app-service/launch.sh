...
if [ "$MVN_ARG_LINE" != "" ] ; then
    ...
    "$mvnBin" -v
    echo
    projects=( "*-model" "*-kjar" "rht-decision-app-service")
...
if [[ "$@" =~ "docker" ]]; then
    ...
    docker run -d -p 8090:8090 --name rht-decision-app-service apps/rht-decision-app-service:1.0-SNAPSHOT
elif [[ "$@" =~ "openshift" ]]; then
    echo "Launching the application on OpenShift..."
    
    oc new-app rht-decision-app-service:1.0-SNAPSHOT
    oc expose svc/rht-decision-app-service
else

	echo "Launching the application locally..."
	pattern="rht-decision-app-service"
	files=( $pattern )
	cd ${files[0]}
