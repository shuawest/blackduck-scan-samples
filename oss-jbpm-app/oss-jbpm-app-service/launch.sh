...
if [ "$MVN_ARG_LINE" != "" ] ; then
    ...
    "$mvnBin" -v
    echo
    projects=( "*-model" "*-kjar" "oss-jbpm-app-service")
...
if [[ "$@" =~ "docker" ]]; then
    ...
    docker run -d -p 8090:8090 --name oss-jbpm-app-service apps/oss-jbpm-app-service:1.0-SNAPSHOT
elif [[ "$@" =~ "openshift" ]]; then
    echo "Launching the application on OpenShift..."
    
    oc new-app oss-jbpm-app-service:1.0-SNAPSHOT
    oc expose svc/oss-jbpm-app-service
else

	echo "Launching the application locally..."
	pattern="oss-jbpm-app-service"
	files=( $pattern )
	cd ${files[0]}
