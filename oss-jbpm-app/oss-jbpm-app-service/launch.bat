...
if "%*" == "" (
	...
	for %%s in ("-model" "-kjar" "oss-jbpm-app-service") do (
...
:startapp
	if not x%mavenInput:docker=%==x%mavenInput% (
		...
		call docker run -d -p 8090:8090 --name oss-jbpm-app-service apps/oss-jbpm-app-service:1.0-SNAPSHOT
	) else if not x%mavenInput:openshift=%==x%mavenInput% (
		echo Launching the application on OpenShift...
		call oc new-app oss-jbpm-app-service:1.0-SNAPSHOT
		call oc expose svc/oss-jbpm-app-service
	) else (
		echo "Launching the application locally..."
		setlocal EnableDelayedExpansion
		cd oss-jbpm-app-service
		cd target
