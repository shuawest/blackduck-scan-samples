...
if [ "$MVN_ARG_LINE" != "" ] ; then
    ...
    "$mvnBin" -v
    echo
    projects=( "*-model" "*-kjar" "oss-drools-app-service")
...
if [[ "$@" =~ "docker" ]]; then
    ...
    docker run -d -p 8090:8090 --name oss-drools-app-service apps/oss-drools-app-service:1.0-SNAPSHOT
elif [[ "$@" =~ "openshift" ]]; then
    echo "Launching the application on OpenShift..."
    
    oc new-app oss-drools-app-service:1.0-SNAPSHOT
    oc expose svc/oss-drools-app-service
else

	echo "Launching the application locally..."
	pattern="oss-drools-app-service"
	files=( $pattern )
	cd ${files[0]}
