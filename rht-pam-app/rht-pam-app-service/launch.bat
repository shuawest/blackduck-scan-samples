...
if "%*" == "" (
	...
	for %%s in ("-model" "-kjar" "rht-pam-app-service") do (
...
:startapp
	if not x%mavenInput:docker=%==x%mavenInput% (
		...
		call docker run -d -p 8090:8090 --name rht-pam-app-service apps/rht-pam-app-service:1.0-SNAPSHOT
	) else if not x%mavenInput:openshift=%==x%mavenInput% (
		echo Launching the application on OpenShift...
		call oc new-app rht-pam-app-service:1.0-SNAPSHOT
		call oc expose svc/rht-pam-app-service
	) else (
		echo "Launching the application locally..."
		setlocal EnableDelayedExpansion
		cd rht-pam-app-service
		cd target
