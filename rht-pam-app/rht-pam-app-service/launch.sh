...
if [ "$MVN_ARG_LINE" != "" ] ; then
    ...
    "$mvnBin" -v
    echo
    projects=( "*-model" "*-kjar" "rht-pam-app-service")
...
if [[ "$@" =~ "docker" ]]; then
    ...
    docker run -d -p 8090:8090 --name rht-pam-app-service apps/rht-pam-app-service:1.0-SNAPSHOT
elif [[ "$@" =~ "openshift" ]]; then
    echo "Launching the application on OpenShift..."
    
    oc new-app rht-pam-app-service:1.0-SNAPSHOT
    oc expose svc/rht-pam-app-service
else

	echo "Launching the application locally..."
	pattern="rht-pam-app-service"
	files=( $pattern )
	cd ${files[0]}
